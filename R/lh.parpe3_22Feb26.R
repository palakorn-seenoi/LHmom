for (eta in 0:4){
  ...
  cat("eta, para=",eta,z$para,z$tau.diff,"\n")
  
  z=lh.parpe3(data,eta, opt=TRUE)
  cat("eta, para=",eta,z$para,z$tau.diff,"\n","\n")
}

#---------------------------------------------------------
lh.parpe3 = function(data, eta=1,  
                     opt=FALSE, ntry=10){
  ...
  if(eta==0){
    ...
    z$eta=0
    z$ifail=0
    z$tau.diff= abs(lhmom.pe3(z,eta)$ratios[4]-t4)
    z$t34.diff= ( abs(lhmom.pe3(z,eta)$ratios[3]-t3)
                  + z$tau.diff )
    z$source="lh.parpe3"
    return(z)
  }
  ...
  if (ifail != 0) {
    if(ifail==2){
      ...
      tau.diff= abs(lhmom.pe3(z,eta)$ratios[4]-t4)
      t34.diff= ( abs(lhmom.pe3(z,eta)$ratios[3]-t3)
                  + tau.diff )
      return(list(model = "pe3", para = para, eta=eta, 
            type="pe3",tau.diff=tau.diff, ifail=ifail,
 ...
 # xi    <- mu - (alpha * be)
  ...
  tau.diff= abs(lhmom.pe3(c(xi,alpha,be),eta)$ratios[4]-t4)
  
  t34.diff= ( abs(lhmom.pe3(c(xi,alpha,be),eta)$ratios[3]-t3)
              + tau.diff )
...
#    warning(paste("Integration failed for S =", S, ":", e$message))
    return(NA)
  })
...
obj.lhpe3 <- function(x, t3=t3, eta=eta) { 
  ...
  #  alpha=x
  tau3= abs(lhmom.pe3(c(0,x,1),eta)$ratios[3])
