library(lmomco)
library(nleqslv)

for(eta in 0:4){
  cat("eta,para k3d=",eta,lh.park3h(data,eta, 
                             hfix=0)$para,"\n")
  cat("eta,para gev=",eta,lh.pargev(data,eta)$para,"\n","\n")
}

#------------------------------------------------
lh.pargpa = function(data,eta=1){
  
  z=list()
  z=lh.park3h(data,eta,hfix=1)
  z$para= z$para[1:3]
  z$type="gpa"
  z$source="lh.pargpa"
  z
}
#------------------------------------------------
lh.parglo = function(data,eta=1){
  
  z=list()
  z=lh.park3h(data,eta,hfix=-1)
  z$para= z$para[1:3]
  z$type="glo"
  z$source="lh.parglo"
  z
}

#---------------------------------------------------------
lh.park3h = function(data, eta=1, hfix=NULL, 
                     opt=FALSE, ntry=10){
  
  z=list()
  if(length(eta) != 1) stop("eta should be a scalar")
  if( eta %% 1 != 0 | eta < 0 | eta >= 5) {
    stop("eta must be a nonnegative integer less than 5")}
  
  h=hfix
  if( h < -eta -1 | h > 5) stop("hfix out of bound")
  small= 1e-6
  
  if(h==0) hfix= h= -small
  if(h != 0 & abs(h) < small) hfix=h=sign(h)*small
  
  samlh=lhmoms(data, eta=eta)
  t3= samlh$ratios[3]
  t4= samlh$ratios[4]
  
  if(eta==0){
    z= park3h(lmoms(data), hfix=hfix)
    z$eta=0
    z$hfix=hfix
    z$tau.diff= abs(lhmom.kap(z$para,eta)$ratios[4]-t4)
    z$t34.diff= ( abs(lhmom.kap(z$para,eta)$ratios[3]-t3)
                + z$tau.diff )
    z$source="lh.park3h"
    return(z)
  }
  
  init= initk(data, model="gev",ntry=ntry)

  # ----- solve using nleqslv  -----
  itry = 1
  ifail = 10
  mysol = NULL
  para=rep(NA,4)
  ftol=ftolz= small
  xtol=xtolz= 10*small
  
  while (itry <= ntry) {
    
    mysol = nleqslv(x = init[itry],
                    fn = obj.lhk3h, method="Broyden",
                    control = list(maxit = 1000,
                              ftol =ftol, xtol =xtol,
                              allowSingular = TRUE),
                    t3=t3, eta=eta, hfix=hfix)
    
    if (abs(mysol$fvec) < ftol| mysol$termcd ==1){
      ifail = 0
      para[3] = mysol$x
      fvec = mysol$fvec
      break
      
    }else{
      itry = itry + 1
      ftol=itry*ftolz
      xtol=itry*xtolz
      
      if(mysol$termcd==2){ #& mysol$x[2] < -0.99 &
        #mysol$x[2] > -1.01){
        ifail=2
        sol=mysol$x
        fvec=mysol$fvec
      }
      if (itry > ntry) {
        #        cat("no LH solution found for K4D")
        if(ifail != 2) ifail = 5
        break
      }
    } #end if sum
  } #end while
  
  if (ifail != 0) {
    if(ifail==2){
      para[3]=sol
    }else{
      z$mysol = mysol
      cat("failure to solve LHme for K3D, at eta=",
          eta,"\n")
      cat("We use khat of GEV, with h=hfix","\n")
      para[3]=lh.pargev(data,eta)$para[3]
    }
  }
  
  L= samlh$lambdas[1:2]
  k=para[3]
  para[4]=hfix
  
  if(h < -eta -1) h= -eta-1 + small
  if(k < -1) k=-0.99999

  if(para[4] <0){
    num=2*L[2]*k* (-h)^(k+1)
    dem= (eta+2)* ( (eta+1)*beta(k+1,(-eta-1)/h -k)
                    -(eta+2)*beta(k+1,(-eta-2)/h -k) )
    # gamma(k+1)*( (eta+1)*G[1]-(eta+2)*G[2] ) 
    sigma = num/dem
    fac= 1- ( (eta+1)*beta(k+1,(-eta-1)/h -k) / ((-h)^(k+1)) )
    mu= L[1]-fac*sigma/k
    
  }else if(para[4] >0){
    num=2*L[2]*k* (h)^(k+1)
    dem= (eta+2)*( (eta+1)*beta(k+1,(eta+1)/h)
                   -(eta+2)*beta(k+1,(eta+2)/h) )
    sigma = num/dem
    fac= 1- ( (eta+1)*beta(k+1,(eta+1)/h) / (h)^(k+1) )
    mu= L[1]-fac*sigma/k
  } # end if
  
  para=c(mu, sigma, k, h)
  
  tau.diff= abs(lhmom.kap(para,eta)$ratios[4]-t4)
  
  t34.diff= ( abs(lhmom.kap(para,eta)$ratios[3]-t3)
              + tau.diff )
  
  return(list(model = "k3d.hfix", para = para, eta=eta, 
              hfix=hfix, type="kap",
              tau.diff=tau.diff, ifail=ifail,
              precision=fvec, t34.diff=t34.diff,
              source="lh.park3h"))
}

# x=as.vector(init[2, 1:2])
#----------------------------------------------------
obj.lhk3h <- function(x, t3=t3, eta=eta, hfix=hfix) { 
  
#  k=x
  if ( x <= -1 ) return(10^6 )   # k for hosking style
  if ( hfix < 0 & (x*hfix) <= -eta-1) return(10^6) 
  #  if( k+0.725*h <= -1) return(c(10^6,10^6) ) 
  
  tau=rep(NA,4)
  tau[3:4]= lhmom.kap(c(0,1,x,hfix),eta)$ratios[3:4]
  
  if( any(is.na(tau[3:4])) ) return(10^6)
  if( tau[4] < (5*tau[3]^2-1)/4 ) return(10^6) 
  
#  tau3= lhmom.kap(c(0,1,x,hfix),eta)$ratios[3]
  
  return(tau[3]-t3)  
}
#---------------------------------------------------- 
